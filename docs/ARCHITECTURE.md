...
- **`region_select.zig`** - the "Start Region Selection" drag-to-select overlay for `RegionFit` layout mode: an interactive (non-click-through) layered window spanning the full virtual desktop, built on `gdi_overlay.zig`'s DIB/`UpdateLayeredWindow` machinery but with a real `WndProc` (unlike the click-through ghost/hint overlays in `painter.zig`). Runs in the main app process since that's where thumbnail placement lives; publishes its result to `config.exe` via `protocol.RegionSelectResult`.
