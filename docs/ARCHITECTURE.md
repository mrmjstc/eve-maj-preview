...
- **`keyboard_hook.zig`** - a `WH_KEYBOARD_LL` hook matching keyboard chords (including bare-modifier triggers like plain "Shift", which a keyboard-API-based approach can't represent) against bound combos and re-posting matches as synthetic `WM_HOTKEY` messages; also swallows a hotkey's key-up after a successful action changes focus, so the newly-focused window doesn't see a phantom key-up.
