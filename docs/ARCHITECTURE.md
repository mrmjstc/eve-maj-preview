...
- **`input.zig`** - client activation (`handleThumbnailClick` - restore, foreground, dismiss suppressible notifications, reconcile focus state), shift-click exclusion toggling, the thumbnail/text-window `WndProc`s (drag, edge/thumbnail/ghost-position snapping), and a low-level keyboard hook that swallows a hotkey's key-up after a successful cycle action. The same hook flags whether a tracked hotkey's key-down was synthetic (`LLKHF_INJECTED`, e.g. a joystick/gamepad key-mapper); for those, a poll timer re-fires `WM_HOTKEY` while held, since synthetic key-downs don't get OS auto-repeat the way physical ones do.
