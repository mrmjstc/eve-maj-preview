...
- **`gdi_overlay.zig`** - the shared GDI leaf module: `OverlayBitmap` (a top-down DIBSection selected into a memory DC), alpha-fixup helpers for GDI text output, pixel-buffer `fillRect`, buffer-truncating `toBufZ`, and `registerWindowClass` (the common `WNDCLASSEXA` registration boilerplate). Used by `painter.zig`, `list_view.zig`, `notif_info_view.zig`, `main.zig`, and `region_select.zig`.
