...
They are not spawned from a shared root - `config.exe` is launched as a child process via `ShellExecuteA` when the user opens it from the tray menu (`tray.zig` `openConfigDialog`). Once running, the two processes are independent and talk over Win32 IPC: `config_dialog.zig` locates the running main app with `FindWindowA` against the timer window's class name (`"EVE_TIMER_CLASS"`), then sends commands via `WM_COPYDATA`/`WM_PROTOCOL_HOTKEY` (see [protocol.zig](#protocolzig-and-cross-process-control)). This is how live profile switching and live thumbnail-appearance preview from the config dialog reach the running app. That channel is one-way (dialog → app); the cases needing app → dialog data instead use small named shared-memory mappings the dialog polls, since building a second `WM_COPYDATA`-style window in the dialog process wasn't worth it: `protocol.RegionSelectResult` (the rect a user just drag-selected for the Thumbnail Space region) and a group membership revision counter (`protocol.bumpGroupMembershipRevision`), bumped whenever an assign key saves a hotkey group's members so an open dialog reloads those member lists before its next Save can overwrite them.
...
- **`gdi_overlay.zig`** - the shared GDI leaf module: `OverlayBitmap` (a top-down DIBSection selected into a memory DC), alpha-fixup helpers for GDI text output, pixel-buffer `fillRect`, buffer-truncating `toBufZ`/`measureTextWidth`/`truncateTextToFit`, `registerWindowClass` (the common `WNDCLASSEXA` registration boilerplate), and the draggable-panel trio `panelHeaderHitTest`/`beginPanelDrag`/`updatePanelDragRect` shared by `list_view.zig`'s and `notif_info_view.zig`'s WM_NCHITTEST/WM_ENTERSIZEMOVE/WM_MOVING handlers. Used by `painter.zig`, `list_view.zig`, `notif_info_view.zig`, `main.zig`, and `region_select.zig`.
- **`fonts.zig`** - embeds the bundled Cascadia Code/Mono TTFs and registers them process-privately (`AddFontMemResourceEx`) at startup, so GDI font lookups work on installs without Cascadia.
...
- **`hotkeys.zig`** - `HotkeyManager` owns a single `hotkey_map: AutoHashMap(c_int, HotkeyAction)`. Hotkey IDs are banded by range (cycle-groups, global actions, per-character, profile-switch, app, URL) so a single `WM_HOTKEY` ID reverse-maps to an action. `handleHotkeyPress()` is the one dispatch point, gating on suspend/focus-requirement state before switching on the `HotkeyAction` union.
- **`keyboard_hook.zig`** - a `WH_KEYBOARD_LL` hook matching keyboard chords (including bare-modifier triggers like plain "Shift", which a keyboard-API-based approach can't represent) against bound combos and re-posting matches as synthetic `WM_HOTKEY` messages; also swallows a hotkey's key-up after a successful action changes focus (unconditionally for the Win key, since Windows treats an unmatched key-up as a Start Menu press regardless of focus) so the newly-focused window doesn't see a phantom key-up. `armWinKeyCapture()`/`disarmWinKeyCapture()` additionally keep the hook alive and swallow Win down/up unconditionally while the config dialog is recording a new binding (which otherwise unregisters everything), reporting the capture to the dialog since a swallowed key never reaches its own DOM listeners.
- **`mouse_hook.zig`** - a `WH_MOUSE_LL` hook mapping mouse-button chords to hotkey IDs and re-posting them as synthetic `WM_HOTKEY` messages, mirroring `keyboard_hook.zig`'s shape so `hotkeys.zig` doesn't need a separate input path for mouse buttons.
- **`input.zig`** - client activation (`handleThumbnailClick` - restore, foreground, dismiss suppressible notifications, reconcile focus state), shift-click exclusion toggling, and the thumbnail/text-window `WndProc`s (drag, edge/thumbnail/ghost-position snapping). Also owns the "focus grant" workaround for `SetForegroundWindow` silently refusing to switch focus unless the caller "just received user input" (true for a real click on our own window, not for a hotkey-triggered or auto-minimize-triggered refocus): a permanently-registered, physically unreachable `RegisterHotKey` binding (unused vk, every modifier combo) stays alive purely to borrow its foreground-lock exemption on demand - `forceSetForegroundWindow()` injects a synthetic keypress matching that binding, and the resulting genuine `WM_HOTKEY` (handled first in `timerWindowProc` via `handleFocusGrantWmHotkey()`, ahead of `hotkeys.zig`'s dispatch) does the actual `SetForegroundWindow` call.
- **`protocol.zig` and cross-process control** - parses `evemajpreview://action/params` URLs, handles registry registration/unregistration, and implements the actual IPC transport (`sendCommandToInstance`, `findExistingInstance`) used both by external protocol invocations and by `config.exe`. Also owns the app→dialog reply channels (`RegionSelectResult`, `WinKeyCaptureResult`), each a named shared-memory mapping with a sequence counter so a poller can tell "no new result" from "stale result from a previous run" - see `region_select.zig` and `keyboard_hook.zig`'s `armWinKeyCapture`.
...
- **`region_select.zig`** - the "New Thumbnail Region" / "Edit Region" overlay (drag a rectangle, then adjust its edges and confirm with Save/Cancel), shared by `RegionFit` layout mode and the not-logged-in Thumbnail Space: an interactive (non-click-through) layered window spanning the full virtual desktop, built on `gdi_overlay.zig`'s DIB/`UpdateLayeredWindow` machinery but with a real `WndProc` (unlike the click-through ghost/hint overlays in `painter.zig`). It's agnostic to which rectangle it's filling in - `config_dialog.js` decides that by which fields it writes the result into, and passes the region to edit, whether to hide thumbnails, and the translated overlay text (the main app has no language files) in the request it sends (`protocol.RegionSelectRequest`). Runs in the main app process since that's where thumbnail placement lives; publishes its result to `config.exe` via `protocol.RegionSelectResult`.
