...
- **`update.zig`** - checks GitHub Releases by shelling out to `curl` on a background thread; opens the release page in a browser.
...
- **`notif_info_view.zig`** - an always-available panel (`display.showNotifInfoPanel`, independent of `viewMode`), structurally a near-twin of `list_view.zig`: notification history (newest-first, click a row to jump to that character) over fleet-wide bounty/mining session totals. History lives in a fixed-capacity ring buffer on `Painter` (`notification_history`, pushed from `showNotification`); totals live in `activity_tracker.zig`'s `MiningWindow`/`BountyWindow` (`total_m3`/`total_isk`, summed per-tick by `main.zig` via `getGrandTotal(s)` and pushed into `Painter.updateActivityTotals`) - they reset per character on logout since `removeCharacter` drops that character's tracker window entirely.
