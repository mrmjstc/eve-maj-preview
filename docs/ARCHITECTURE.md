...
- **`region_select.zig`** - the "Start Region Selection" drag-to-select overlay, shared by `RegionFit` layout mode and the not-logged-in Thumbnail Space: an interactive (non-click-through) layered window spanning the full virtual desktop, built on `gdi_overlay.zig`'s DIB/`UpdateLayeredWindow` machinery but with a real `WndProc` (unlike the click-through ghost/hint overlays in `painter.zig`). It's agnostic to which rectangle it's filling in - `config_dialog.js` decides that by which fields it writes the result into. Runs in the main app process since that's where thumbnail placement lives; publishes its result to `config.exe` via `protocol.RegionSelectResult`.
