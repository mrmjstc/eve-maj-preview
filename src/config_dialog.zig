...
fn mainImpl(init: std.process.Init) !void {
    ...
    if (config_mod.GlobalSettings.load(allocator)) |loaded| {
        ...
        slog.warn("Failed to load global settings for dialog startup: {}", .{err});
    }
...
fn revealDialogWindow(win: anytype, initial_always_on_top: bool) void {
    ...
    if (g_dialog_hwnd) |hwnd| {
        std.Io.sleep(g_io, .fromMilliseconds(1000), .awake) catch |err| {
            slog.debug("Diagnostic sleep failed: {}", .{err});
        };
...
fn getUpdateStatus(e: *webui.Event) void {
    ...
    if (!appendJsonEscaped(allocator, &response, version)) {
        e.returnString("{\"available\": false}");
        return;
    }
    ...
    if (!appendJsonEscaped(allocator, &response, url)) {
        e.returnString("{\"available\": false}");
        return;
    }
    ...
    if (notes) |n| {
        if (!appendJsonEscaped(allocator, &response, n)) {
            e.returnString("{\"available\": false}");
            return;
        }
    }
...
fn loadGlobalSettings(e: *webui.Event) void {
    ...
    if (parsed.value == .object) {
        ...
        for (config_mod.DEFAULT_ORE_TABLE) |default_entry| {
            ...
            obj.put(arena_allocator, "name", .{ .string = default_entry.name }) catch |err| {
                slog.warn("Failed to set oreTable name for {s}: {}", .{ default_entry.name, err });
                continue;
            };
            obj.put(arena_allocator, "category", .{ .string = default_entry.category }) catch |err| {
                slog.warn("Failed to set oreTable category for {s}: {}", .{ default_entry.name, err });
                continue;
            };
            obj.put(arena_allocator, "volumeM3", .{ .float = default_entry.volumeM3 }) catch |err| {
                slog.warn("Failed to set oreTable volumeM3 for {s}: {}", .{ default_entry.name, err });
                continue;
            };
            obj.put(arena_allocator, "price", .{ .float = price }) catch |err| {
                slog.warn("Failed to set oreTable price for {s}: {}", .{ default_entry.name, err });
                continue;
            };
            display_ore.append(.{ .object = obj }) catch |err| {
                slog.warn("Failed to append oreTable entry for {s}: {}", .{ default_entry.name, err });
                continue;
            };
        }
        parsed.value.object.put(arena_allocator, "oreTable", .{ .array = display_ore }) catch |err| {
            slog.warn("Failed to merge oreTable into global settings response: {}", .{err});
        };
    }
...
fn enumClientsCallback(hwnd: win32.HWND, lParam: win32.LPARAM) callconv(.c) win32.BOOL {
    ...
    const name_copy = ctx.allocator.dupe(u8, char_name) catch |err| {
        slog.warn("Failed to copy client name '{s}': {}", .{ char_name, err });
        return win32.TRUE;
    };
    ctx.entries.append(ctx.allocator, .{ .name = name_copy, .creation_time = creation_time }) catch |err| {
        slog.warn("Failed to record client '{s}': {}", .{ char_name, err });
...
fn getOpenClients(e: *webui.Event) void {
    ...
    for (ctx.entries.items, 0..) |entry, i| {
        if (i > 0) response.append(allocator, ',') catch {
            truncated = true;
            break;
        };
        response.append(allocator, '"') catch {
        ...
        if (!appendJsonEscaped(allocator, &response, entry.name)) {
            truncated = true;
            break;
        }
        response.append(allocator, '"') catch {
    ...
    if (truncated) slog.warn("Open clients response truncated (out of memory)", .{});
...
fn enumRunningWindowsCallback(hwnd: win32.HWND, lParam: win32.LPARAM) callconv(.c) win32.BOOL {
    ...
    const class_copy = ctx.allocator.dupe(u8, class_slice) catch |err| {
        slog.warn("Failed to copy window class '{s}': {}", .{ class_slice, err });
        return win32.TRUE;
    };
    const exe_copy = ctx.allocator.dupe(u8, exe_name) catch |err| {
        slog.warn("Failed to copy executable name '{s}': {}", .{ exe_name, err });
        ctx.allocator.free(class_copy);
        return win32.TRUE;
    };
    const title_copy = ctx.allocator.dupe(u8, title_slice) catch |err| {
        slog.warn("Failed to copy window title '{s}': {}", .{ title_slice, err });
        ctx.allocator.free(class_copy);
        ctx.allocator.free(exe_copy);
        return win32.TRUE;
    };

    ctx.entries.append(ctx.allocator, .{ .class_name = class_copy, .executable_name = exe_copy, .title = title_copy }) catch |err| {
        slog.warn("Failed to record running window '{s}': {}", .{ title_slice, err });
        ctx.allocator.free(class_copy);
        ctx.allocator.free(exe_copy);
...
/// Returns false (with the response left partially escaped) if a write failed, so callers can treat it the same as their other append failures.
fn appendJsonEscaped(allocator: std.mem.Allocator, response: *std.ArrayList(u8), s: []const u8) bool {
    const hex_digits = "0123456789abcdef";
    for (s) |c| {
        switch (c) {
            '"' => response.appendSlice(allocator, "\\\"") catch return false,
            '\\' => response.appendSlice(allocator, "\\\\") catch return false,
            '\n' => response.appendSlice(allocator, "\\n") catch return false,
            '\r' => response.appendSlice(allocator, "\\r") catch return false,
            '\t' => response.appendSlice(allocator, "\\t") catch return false,
            0x00...0x08, 0x0B, 0x0C, 0x0E...0x1F => {
                response.appendSlice(allocator, "\\u00") catch return false;
                response.append(allocator, hex_digits[c >> 4]) catch return false;
                response.append(allocator, hex_digits[c & 0xF]) catch return false;
            },
            else => response.append(allocator, c) catch return false,
        }
    }
    return true;
}
...
fn getRunningWindows(e: *webui.Event) void {
    ...
    for (ctx.entries.items, 0..) |entry, i| {
        ...
        response.appendSlice(allocator, "{\"class\":\"") catch {
            truncated = true;
            break;
        };
        if (!appendJsonEscaped(allocator, &response, entry.class_name)) {
            truncated = true;
            break;
        }
        response.appendSlice(allocator, "\",\"exe\":\"") catch {
            truncated = true;
            break;
        };
        if (!appendJsonEscaped(allocator, &response, entry.executable_name)) {
            truncated = true;
            break;
        }
        response.appendSlice(allocator, "\",\"title\":\"") catch {
            truncated = true;
            break;
        };
        if (!appendJsonEscaped(allocator, &response, entry.title)) {
            truncated = true;
            break;
        }
        response.appendSlice(allocator, "\"}") catch {
    ...
    if (truncated) slog.warn("Running windows response truncated (out of memory)", .{});
...
fn scaleLegacyPositions(e: *webui.Event) void {
    ...
    for (parsed.value, 0..) |pos, i| {
        ...
        const entry = std.fmt.bufPrint(&buf, "{{\"x\":{},\"y\":{}}}", .{ scaled.x, scaled.y }) catch {
            truncated = true;
            break;
        };
        response.appendSlice(allocator, entry) catch {
            truncated = true;
            break;
        };
    }
    if (truncated) slog.warn("Scaled positions response truncated (out of memory)", .{});
...
fn scanUltraPotatoProfiles(e: *webui.Event) void {
    ...
    for (profiles, 0..) |p, i| {
        ...
        response.appendSlice(allocator, "{\"path\":\"") catch {
            truncated = true;
            break;
        };
        if (!appendJsonEscaped(allocator, &response, p.path)) {
            truncated = true;
            break;
        }
        response.appendSlice(allocator, "\",\"label\":\"") catch {
            truncated = true;
            break;
        };
        if (!appendJsonEscaped(allocator, &response, p.label)) {
            truncated = true;
            break;
        }
        response.appendSlice(allocator, "\"}") catch {
    ...
    if (truncated) slog.warn("Ultra Potato profile list response truncated (out of memory)", .{});
...
fn applyUltraPotatoMode(e: *webui.Event) void {
    ...
    for (results, 0..) |r, i| {
        ...
        if (!appendJsonEscaped(allocator, &response, r.path)) {
            truncated = true;
            break;
        }
        response.appendSlice(allocator, "\",\"ok\":") catch {
            truncated = true;
            break;
        };
        response.appendSlice(allocator, if (r.success) "true" else "false") catch {
            truncated = true;
            break;
        };
        response.appendSlice(allocator, ",\"changed\":") catch {
            truncated = true;
            break;
        };
        response.appendSlice(allocator, if (r.changed) "true" else "false") catch {
            truncated = true;
            break;
        };
        response.appendSlice(allocator, ",\"error\":") catch {
            truncated = true;
            break;
        };
        if (r.error_message) |m| {
            ...
            if (!appendJsonEscaped(allocator, &response, m)) {
                truncated = true;
                break;
            }
            response.append(allocator, '"') catch {
            ...
            response.appendSlice(allocator, "null") catch {
                truncated = true;
                break;
            };
        }
    ...
    if (truncated) slog.warn("Ultra Potato apply-results response truncated (out of memory)", .{});
...
fn listProfiles(e: *webui.Event) void {
    ...
    for (profiles.items, 0..) |profile, i| {
        ...
        if (!appendJsonEscaped(allocator, &response, profile)) {
            truncated = true;
            break;
        }
        response.append(allocator, '\"') catch {
    ...
    if (truncated) slog.warn("Profile list response truncated (out of memory)", .{});

    response.appendSlice(allocator, "], \"current\": \"") catch {
        e.returnString("{\"error\": \"Failed to build response\"}");
        return;
    };
    if (!appendJsonEscaped(allocator, &response, current_profile)) {
        e.returnString("{\"error\": \"Failed to build response\"}");
        return;
    }
    response.appendSlice(allocator, "\"}") catch {
...
fn listProfileBackups(e: *webui.Event) void {
    ...
    while (true) {
        const entry = iter.next(g_io) catch |err| {
            slog.warn("Failed to enumerate profile backups directory: {}", .{err});
            break;
        } orelse break;
        if (entry.kind == .file and std.mem.endsWith(u8, entry.name, ".json")) {
            const backup_name = allocator.dupe(u8, entry.name) catch |err| {
                slog.warn("Failed to copy backup name '{s}': {}", .{ entry.name, err });
                continue;
            };
            backups.append(allocator, backup_name) catch |err| {
                slog.warn("Failed to record backup '{s}': {}", .{ entry.name, err });
    ...
    for (backups.items, 0..) |backup, i| {
        ...
        if (!appendJsonEscaped(allocator, &response, backup)) {
            truncated = true;
            break;
        }
        response.append(allocator, '\"') catch {
    ...
    if (truncated) slog.warn("Profile backups response truncated (out of memory)", .{});
