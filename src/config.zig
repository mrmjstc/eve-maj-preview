...
pub const CombatConfig = struct {
    ...
    pub fn deinit(self: *CombatConfig, allocator: std.mem.Allocator) void {
        ...
        freeFontNameIfOwned(allocator, self.incoming_font_name);
        freeFontNameIfOwned(allocator, self.outgoing_font_name);
    }
...
pub const MiningConfig = struct {
    ...
    pub fn deinit(self: *MiningConfig, allocator: std.mem.Allocator) void {
        freeFontNameIfOwned(allocator, self.font_name);
    }
...
pub const Config = struct {
    ...
    fn updateOwnedFontName(allocator: std.mem.Allocator, field: *[]const u8, v: std.json.Value) !void {
        ...
        field.* = try allocator.dupe(u8, v.string);
        freeFontNameIfOwned(allocator, old_font);
    }
    ...
    pub fn deinit(self: *Config) void {
        ...
        freeFontNameIfOwned(allocator, self.thumbnail.characterNameFontName);
    ...
    pub fn reloadThumbnailConfigFromDisk(self: *Config, allocator: std.mem.Allocator) !void {
        ...
        freeFontNameIfOwned(allocator, self.combat.outgoing_font_name);
        allocator.free(self.combat.damage_alert_excluded_weapons);
        self.combat = new_combat;

        freeFontNameIfOwned(allocator, self.mining.font_name);
        self.mining = new_mining;

        freeFontNameIfOwned(allocator, self.bounty.font_name);
        self.bounty = new_bounty;

        freeFontNameIfOwned(allocator, self.resources.font_name);
