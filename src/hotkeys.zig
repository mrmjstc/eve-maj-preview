...
/// Returns HotkeyManager's cached name -> configured-order-index map, rebuilding it only if the character list's names/order changed since the last call. The map only depends on config.characters, which changes far less often (profile reload, config-dialog edits) than cycle hotkeys are pressed.
fn getCharacterOrderMap(self: *HotkeyManager) !*const std.StringHashMap(usize) {
    const sig = characterOrderSignature(self.config.characters.items);
    if (self.character_order_cache == null or self.character_order_cache_signature != sig) {
        if (self.character_order_cache) |*old| old.deinit();
        self.character_order_cache = try config_mod.buildCharacterOrderMap(self.config.characters.items, self.allocator);
        self.character_order_cache_signature = sig;
    }
    return &self.character_order_cache.?;
}
...
fn buildCharacterOrderedIndices(self: *HotkeyManager, windows: []const scout.EveWindow) ![]usize {
    const order_map = try getCharacterOrderMap(self);

    const indices = try self.allocator.alloc(usize, windows.len);
...
pub const HotkeyManager = struct {
    ...
    character_order_cache: ?std.StringHashMap(usize) = null,
    character_order_cache_signature: u64 = 0,
    /// Cache for getExcludedList: the deduplicated union of every group's exclusion list and manually_excluded_characters. Elements borrow from those lists, so only the ArrayList's own backing array is owned here. Rebuilt only when excludedListSignature detects a change.
    excluded_list_cache: std.ArrayList([]const u8) = .empty,
    excluded_list_cache_signature: ?u64 = null,
    ...
    /// Appends names not already in seen to dest, marking them seen; used by getExcludedList() to dedupe.
    ...
    /// Cheap, order-sensitive fingerprint of getExcludedList's inputs (every hotkey group's exclusion list, then manually_excluded_characters), used to detect when the cached list needs rebuilding. Each name is length-prefixed so moving a name across a group boundary can't leave the concatenated byte stream accidentally unchanged.
    fn excludedListSignature(self: *const HotkeyManager) u64 {
        var h = std.hash.Wyhash.init(0);
        for (self.config.hotkeyGroups.items) |*group| {
        ...
        for (self.manually_excluded_characters.items) |name| {
            h.update(std.mem.asBytes(&@as(u32, @intCast(name.len))));
            h.update(name);
        }
        return h.final();
    ...
    /// Returns the cached deduplicated union of every group's exclusion list and manually_excluded_characters, rebuilding only if excludedListSignature detects a change; shared so cycleExcluded/updateExcludedCycleIndex agree. Bounded by human input rate either way, but rebuilding a StringHashMap on every thumbnail click/hotkey activation was needless churn.
    fn getExcludedList(self: *HotkeyManager) *const std.ArrayList([]const u8) {
        const sig = self.excludedListSignature();
        if (self.excluded_list_cache_signature == null or self.excluded_list_cache_signature.? != sig) {
            self.excluded_list_cache.clearRetainingCapacity();

            var seen = std.StringHashMap(void).init(self.allocator);
            defer seen.deinit();

            for (self.config.hotkeyGroups.items) |*group| {
                self.appendUnseenNames(&self.excluded_list_cache, &seen, group.excluded_characters.items);
            }
            self.appendUnseenNames(&self.excluded_list_cache, &seen, self.manually_excluded_characters.items);

            self.excluded_list_cache_signature = sig;
        }
        return &self.excluded_list_cache;
    }
    ...
    fn cycleExcluded(self: *HotkeyManager, forward: bool) void {
        const excluded_list = self.getExcludedList();
    ...
    fn cycleAllClients(self: *HotkeyManager, forward: bool) void {
        ...
        const order = buildCharacterOrderedIndices(self, windows) catch |err| {
    ...
    /// Cycles windows still at the login screen ("EVE" title), in the order they logged out; cursor must be HWND since they all share that name.
    fn cycleNotLoggedIn(self: *HotkeyManager, forward: bool) void {
        var hwnds = self.scout.getNotLoggedInHwnds(self.allocator) catch |err| {
            slog.err("Failed to build not-logged-in window list: {}", .{err});
            return;
        };
        defer hwnds.deinit(self.allocator);
    ...
    pub fn deinit(self: *HotkeyManager) void {
        ...
        if (self.character_order_cache) |*cache| cache.deinit();
        self.excluded_list_cache.deinit(self.allocator);
