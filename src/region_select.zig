...
fn drawBorder(bmp: *const gdi_overlay.OverlayBitmap, x: usize, y: usize, w: usize, h: usize) void {
    ...
    gdi_overlay.fillRect(bmp.pixels, bmp.width, bmp.height, x, y, w, @min(t, h), g_border_color);
    if (h > t) gdi_overlay.fillRect(bmp.pixels, bmp.width, bmp.height, x, y + h - t, w, t, g_border_color);
    gdi_overlay.fillRect(bmp.pixels, bmp.width, bmp.height, x, y, @min(t, w), h, g_border_color);
    if (w > t) gdi_overlay.fillRect(bmp.pixels, bmp.width, bmp.height, x + w - t, y, t, h, g_border_color);
}
...
fn redraw() void {
    ...
    if (g_dragging) {
        ...
        if (w > 0 and h > 0) {
            ...
            gdi_overlay.fillRect(bmp.pixels, bmp.width, bmp.height, local_x, local_y, uw, uh, CLEAR_COLOR);
