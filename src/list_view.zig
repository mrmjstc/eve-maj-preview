...
pub const ListWindow = struct {
    ...
    pub fn render(self: *ListWindow, all_thumbnails: []const ThumbnailWindow, active_source_hwnd: ?win32.HWND) !void {
        ...
        for (render_order, 0..) |thumb_index, i| {
            ...
            gdi_overlay.fillRect(ov.pixels, W, H, col_left_u, row_top_u, @intCast(LIST_WIDTH), @intCast(ROW_HEIGHT), row_bg);
            ...
            if (row + 1 < itemsInColumn(n, columns, col)) {
                gdi_overlay.fillRect(ov.pixels, W, H, col_left_u, row_top_u + @as(usize, @intCast(ROW_HEIGHT)) - 1, @intCast(LIST_WIDTH), 1, ARGB_SEPARATOR);
            }
        ...
        while (sep_col < columns) : (sep_col += 1) {
            ...
            gdi_overlay.fillRect(ov.pixels, W, H, x, @intCast(HEADER_HEIGHT), 1, sep_h, ARGB_SEPARATOR);
        }
        ...
        // Outer frame staircased per-column so it hugs each column's content instead of the full (possibly taller) window rect.
        {
            ...
            while (frame_c < columns) : (frame_c += 1) {
                ...
                gdi_overlay.fillRect(ov.pixels, W, H, seg_left, bottom_y, @intCast(LIST_WIDTH), 1, frame_col);
            }

            const last_items = itemsInColumn(n, columns, columns - 1);
            const right_h: usize = @intCast(HEADER_HEIGHT + last_items * ROW_HEIGHT);
            gdi_overlay.fillRect(ov.pixels, W, H, W - 1, 0, 1, right_h, frame_col);
        }
